...
times, probabilities, chosen_states_coupling_subindices = simulate(chosen_states_coupling_labels,chosen_coupling_labels,chosen_pulse_time,chosen_bi,[1,0,0,0],T_STEPS)
...
inset_gs = GridSpec(2, 1, left=0.7, bottom=0.35, right=0.95, top=0.85, hspace=0.1)
...
rect = matplotlib.patches.Rectangle((0.7, 0.35), 0.25, 0.5, facecolor='white', edgecolor='none', transform=fig.transFigure, figure=fig,zorder=0)
fig.patches.extend([rect])


axinset3 = ax.inset_axes([0.35, 0.6, 0.2, 0.3])
...
# %%
# Plot results
fig,(axinset,ax,axinset2) = plt.subplots(3,1,figsize=(6.5,2.5),zorder=-2,constrained_layout=True,height_ratios=(1,2,1))
axinset2.set_xlabel('$t\,(\mu s)$')
ax.set_ylabel('$P_i$')
ax.set_ylim(0,1.0)
ax.set_xlim(0,global_pulse_time*2*1e6)
# ax.set_xlim(0,5000)
...
axinset3 = ax.inset_axes([0.7, 0.00, 0.3, 1.0])
# axinset2.plot(times*1e6,probabilities[:,:],c='grey',linewidth=0.5,alpha=0.5);
for i,state_subindex in enumerate(chosen_states_coupling_subindices):
    axinset3.plot(times*1e6,probabilities[:,state_subindex],c=c[i],linewidth=1.0);
axinset3.set_xlim(500,600)
...
axinset3.axhline(0.25+0.001,color='black',linestyle='dashed',lw=1)
axinset3.axhline(0.25-0.001,color='black',linestyle='dashed',lw=1)
axinset3.yaxis.tick_right()
axinset3.yaxis.set_label_position("right")
